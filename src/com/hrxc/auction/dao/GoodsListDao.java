...
public class GoodsListDao {
    ...
    public GoodsList getObjectById(String pkId) throws SQLException {
        Connection conn = null;
        QueryRunner queryRunner = null;
        GoodsList noteInfo = null;
        String sql = SQL_BASIC_QUERY.concat(" and pk_id=?");
        try {
            conn = JdbcUtil.getConn();
            queryRunner = new QueryRunner();
            noteInfo = (GoodsList) queryRunner.query(conn, sql, new BeanHandler(GoodsList.class), pkId);
        } finally {
            DbUtils.close(conn);
        }
        return noteInfo;
     ...
     * @throws SQLException
     */
    public void saveOrUpdateNoteInfo(GoodsList dto) throws SQLException {
        Connection conn = null;
        QueryRunner queryRunner = null;
        try {
            conn = JdbcUtil.getConn();
